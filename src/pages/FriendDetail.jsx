  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
        ...
        <h3 className="text-[16px] font-extrabold text-[#1a2e22] text-center mb-1">Remove {name}?</h3>
        <p className="text-[13px] text-[#5a7a68] text-center mb-6">
          This will permanently remove them from your friends list. This action cannot be undone.
        </p>
        <div className="flex gap-3">
          ...
          <button
            onClick={onConfirm}
            className="flex-1 py-2.5 rounded-xl bg-red-500 hover:bg-red-600 active:scale-95 text-white font-semibold text-[13px] transition-all"
          >
            Yes, Delete
          </button>
        </div>
...
export default function FriendDetail() {
  ...
  const [goalValue, setGoalValue]             = useState(friend.goal);
  ...
  function handleSnooze() {
    if (snoozed) { toast.info("Already snoozed.", { icon: "⏰" }); return; }
    setSnoozed(true);
    toast.success(`${friend.name.split(" ")[0]} snoozed for 2 weeks. You won't be reminded until then.`, { icon: "⏰" });
  }

  function handleArchive() {
    if (archived) { toast.info("Already archived.", { icon: "📦" }); return; }
    setArchived(true);
    toast.success(`${friend.name.split(" ")[0]} has been archived.`, { icon: "📦" });
  }

  function handleDeleteConfirm() {
    setShowDeleteModal(false);
    toast.success(`${friend.name} has been removed from your friends.`, { icon: "🗑️" });
    setTimeout(() => navigate("/"), 1200);
  }

  function handleSaveGoal() {
    if (!goalValue || goalValue < 1) { toast.error("Goal must be at least 1 day."); return; }
    setEditingGoal(false);
    toast.success(`Goal updated to every ${goalValue} days.`, { icon: "✏️" });
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-5">
          ...
          >
            ...
            <span className={`inline-flex items-center gap-1.5 mt-2 px-2.5 py-0.5 rounded-full text-[11.5px] font-semibold ${cfg.bg} ${cfg.text}`}>
              ...
              {snoozed ? "Snoozed" : archived ? "Archived" : cfg.label}
            </span>
            ...
            <div className="w-full flex flex-col gap-2 border-t border-[#e6ede9] pt-4">
              <button
                ...
                disabled={snoozed}
                className="flex items-center justify-center gap-2 w-full py-2 rounded-xl border border-[#e6ede9] text-[#1a2e22] hover:bg-[#f0f5f2] active:scale-95 text-[12.5px] font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                </svg>
                {snoozed ? "Snoozed ✓" : "Snooze 2 Weeks"}
              </button>
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 w-full py-2 rounded-xl border border-red-100 text-red-500 hover:bg-red-50 active:scale-95 text-[12.5px] font-semibold transition-all"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                </svg>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                ...
                { label: "Due Count",           value: goalValue                 },
            ...
            >
              <div className="flex items-center justify-between mb-3">
                ...
                {!editingGoal ? (
                  <button
                    ...
                    className="text-[12px] font-semibold text-[#2d6a4f] border border-[#c8ddd3] hover:bg-[#f0f5f2] active:scale-95 px-3 py-1 rounded-full transition-all flex items-center gap-1"
                  >
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={handleSaveGoal}
                      className="text-[12px] font-semibold text-white bg-[#1a3a2a] hover:bg-[#122b1f] px-3 py-1 rounded-full transition-colors">
                      Save
                    </button>
                    <button onClick={() => { setEditingGoal(false); setGoalValue(friend.goal); }}
                      className="text-[12px] font-semibold text-[#5a7a68] border border-[#d4e4db] hover:bg-[#f0f5f2] px-3 py-1 rounded-full transition-colors">
                      Cancel
                    </button>
                  </div>
                )}
              ...
              {editingGoal ? (
                <div className="flex items-center gap-3 mb-3">
                  <span className="text-[13px] text-[#5a7a68]">Connect every</span>
                  <input
                    type="number" min="1" max="365"
                    value={goalValue}
                    onChange={e => setGoalValue(Number(e.target.value))}
                    className="w-20 border border-[#c8ddd3] rounded-lg px-3 py-1.5 text-[13px] font-bold text-[#1a2e22] focus:outline-none focus:ring-2 focus:ring-[#2d6a4f] text-center"
                  />
                  <span className="text-[13px] text-[#5a7a68]">days</span>
                </div>
              ) : (
                <p className="text-[13px] text-[#5a7a68] mb-3">
                  Connect every <span className="font-bold text-[#1a2e22]">{goalValue} days</span>
                </p>
              )}
              ...
              <div className="w-full bg-[#e6ede9] rounded-full h-2">
                <div
                  ...
                  style={{ width: `${Math.min((friend.days_since_contact / goalValue) * 100, 100)}%` }}
                />
              </div>
            </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <button
                    ...
                    className={`flex flex-col items-center gap-2 py-4 rounded-xl border bg-white ${bg} ${border} active:scale-95 transition-all duration-200`}
                  >
                    <Icon />
